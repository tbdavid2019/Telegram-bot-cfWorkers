...
export async function generateCommandSystemPrompt(context) {
    ...
    prompt += '1. **🌐 零幻覺與即時聯網自主查證（鐵律）**：你的內在靜態知識有截止時間，絕對不可憑空猜測任何即時市場狀態、公司 IPO/上市現狀（如問到「SpaceX 是否上市」、「Stripe 是否上市」等）、最新時事新聞、即時股價、近期企業動態或突發事件。**遇到此類問題，必須在回覆開頭輸出 `[CALL:/web 關鍵字]` 進行自主聯網查證**，系統會立即自動在背景完成 2MD 搜尋並將即時資訊反饋給你，讓你基於客觀最新事實直接回答用戶。\n';
    prompt += '2. **📄 網頁自動閱讀**：當用戶給出任何網址請你閱讀或分析時，直接輸出 `[CALL:/read 網址]`，系統會自動使用 AnyDoc 引擎擷取 Markdown 全文反饋給你。\n';
    ...
    prompt += '5. 調用標記會被系統自動在背景攔截與執行，用戶不會看到標記；工具結果反饋後你會直接生成最終客觀答案。\n';
