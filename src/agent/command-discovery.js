...
export async function generateCommandSystemPrompt(context) {
    ...
    prompt += '- **📘 David888 Wiki 長文/報導/整理自動發布（LLM 專用長文推送工具）**：當用戶要求「撰寫 2000 字長文報導 / 整理產品手冊 / 深度分析戰爭時事 / 製作英語商務口說對話 / 教學手冊」時，請直接撰寫完整詳實的 Markdown 長文，並主動調用：\n';
    prompt += '  `[CALL:/wiki {"slug": "自訂英文slug", "title": "文章標題", "content": "# 文章主標題\\n\\n[TOC]\\n\\n## 第一章\\n內文..."}]`\n';
    prompt += '  ⚠️ **Wiki 格式鐵律**：content 的第一行必須是 `# 文章主標題`，第二行空一行後放 `[TOC]` 目錄，千萬不可將 `[TOC]` 放第一行！\n';
    ...
    prompt += '2. **📘 長文與產品/時事/教學深度整理自動推送 Wiki**：當用戶提到「透過 wiki 寫... / 撰寫 2000 字長文報導 / 白宮主導的戰爭整理 / 英語商務口說對話 / 產品文檔」，請直接撰寫高質量完整長文（**第一行必須是 `# 大標題`，接著才是 `[TOC]`**），並在第一輪輸出 `[CALL:/wiki {"slug":"...","title":"...","content":"# 大標題\\n\\n[TOC]\\n\\n..."}]` 發布至 David888 Wiki。工具執行成功後，在 Telegram 中只提供簡短精要導讀，並附上專屬公開閱讀連結 (`shareUrl`)、簡報連結 (`/present`) 與電子書連結 (`/book`)，絕不要在 Telegram 重複印出萬字全文。\n';
    ...
    prompt += '6. **多輪自主循環 (ReAct)**：系統支援最多 10 輪的自主工具調用迴圈。若你在獲得第一階段工具結果後，發現需要進一步搜尋其他關鍵字、閱讀特定網址、查詢行情或發布 Wiki，可繼續輸出相應的 `[CALL:指令 參數]`，系統會持續執行並反饋真實數據，直至資訊齊全後再給出最終回答。\n';
