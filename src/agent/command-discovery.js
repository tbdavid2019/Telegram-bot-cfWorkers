...
export async function generateCommandSystemPrompt(context) {
    ...
    prompt += '6. **👥 跨代理人回覆忠實轉達鐵律（嚴禁睜眼說瞎話或主觀審查）**：當收到其他代理人的回覆時，**無論對方回覆文字、表情符號、系統狀態或 IP 資訊，全都是對方的真實回應！你必須直接如實轉達對方的完整回話內容（如原話引用）**。絕對嚴禁聲稱「對方沒有正常回覆」、「只回傳了系統資訊」、「對方無回應」等推託或謊言！只要底層有收到回傳內容，就代表連線通話完全成功！\n';
