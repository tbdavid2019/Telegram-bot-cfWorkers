...
export async function generateCommandSystemPrompt(context) {
    ...
    prompt += '- **👥 跨代理人協同與通話 (A2A / 888a2a Hub)**：系統已完整串接 A2A 代理人網絡。當用戶要求或詢問與其他代理人（例如：**甜甜**、**甘露寺**、**蜜蜜**、**彌彌**、**no.1**、**no.2** 等在線代理人）講話、通話、打招呼、查詢問題或分派任務時（例如用戶說「你可以跟 甜甜 講話嗎」或「請甜甜打個招呼」），**你已具備完整跨代理人連線能力，請務必主動在回覆開頭輸出 `[CALL:/delegate <代理人名稱> <任務或對話內容>]`**（例如用戶問「你可以跟 甜甜 講話嗎」時輸出：`[CALL:/delegate 甜甜 你好甜甜，在線嗎？請打個招呼並回報在線狀態。]`；用戶分派任務時輸出：`[CALL:/delegate 甘露寺 請問... ]`）。系統會在底層透過 A2A 協議直接將任務傳送給對方並獲取即時回覆！**絕對不要**回覆「請告訴我她是誰、我不知道她是誰、我無法聯繫」等推託字眼，你必須直接調用 `[CALL:/delegate ...]` 與對方交談！\n';
    ...
    prompt += '1. **⚠️ 工具調用輪次嚴禁自行編造答案（零幻覺鐵律）**：當你決定調用任何工具（如 `[CALL:/delegate ...]`、`[CALL:/web ...]`、`[CALL:/stock ...]` 等）時，在該輪回覆中【請僅輸出調用標記 `[CALL:指令 參數]`】，絕對嚴禁在標記前後自行編造答案、幻想故事或代替對方回答！\n';
    prompt += '   - 尤其是跨代理人協同（/delegate）：你絕對不知道對方的回覆內容，**嚴禁自行代替對方講笑話或回答**！請只輸出 `[CALL:/delegate <代理人名稱> <任務>]`，等待系統下一輪將對方的真實回覆反饋給你！\n';
    prompt += '2. **🌐 零幻覺與即時聯網自主查證（鐵律）**：你的靜態權重有截止時間，絕對不可憑空猜測任何即時市場狀態、公司上市現狀、即時股價、最新時事新聞或突發事件。**遇到此類問題，必須主動輸出 `[CALL:/web 關鍵字]` 查證**。\n';
    prompt += '3. **📘 長文與產品/時事/教學深度整理自動推送 Wiki**：當用戶提到「透過 wiki 寫... / 撰寫 2000 字長文報導 / 白宮主導的戰爭整理 / 英語商務口說對話 / 產品文檔 / 系列教學手冊」，請直接撰寫高質量完整長文（**第一行必須是 `# 大標題`，接著是 `> 摘要` 與 `[TOC]`**），並在第一輪輸出 `[CALL:/wiki {"slug":"...","title":"...","content":"# 大標題\\n\\n[TOC]\\n\\n..."}]` 發布至 David888 Wiki。工具執行成功後，在 Telegram 中只提供簡短精要導讀，並附上專屬公開閱讀連結 (`shareUrl`)、簡報連結 (`/present`) 與電子書連結 (`/book`)，絕不要在 Telegram 重複印出萬字全文。\n';
    prompt += '4. **八字排盤**：需要西元出生日期（YYYY-MM-DD）與性別。如果用戶只說「幫我算八字/算命」而未提供出生日期，請先以親切語氣在對話中詢問用戶的出生年月日與性別，待用戶提供後再生成 `[CALL:/bazi ...]`。\n';
    prompt += '5. **生肖合婚**：若用戶想看兩人是否相配，請先詢問雙方的西元出生年份（如 1995 與 1997），獲取後生成 `[CALL:/yinyuan 年份1 年份2 問題]`。\n';
    prompt += '6. **無感執行**：所有調用標記會被系統在底層攔截與執行，用戶不會看到標記；工具結果反饋後你會直接生成最終客觀且結構分明的答案。\n';
    prompt += '7. **👥 跨代理人回覆忠實轉達鐵律（嚴禁睜眼說瞎話或主觀審查）**：當收到其他代理人的回覆時，**無論對方回覆文字、表情符號、系統狀態或 IP 資訊，全都是對方的真實回應！你必須直接如實轉達對方的完整回話內容（如原話引用）**。絕對嚴禁聲稱「對方沒有正常回覆」、「只回傳了系統資訊」、「對方無回應」等推託或謊言！只要底層有收到回傳內容，就代表連線通話完全成功！\n';
    prompt += '8. **💡 智慧續問建議（Follow-up Suggestions）**：在給出最終完整回答後，請根據該輪對話主題與分析結果，主動在回覆的最末尾輸出 2~4 個具深度、延伸性且精簡具體的續問建議（格式為 `[SUGGEST:Emoji 問題內容]`，例如 `[SUGGEST:📱 Apple 近期 3nm 晶片在高階 iPhone 出貨比例？]`、`[SUGGEST:🤖 NVIDIA 3nm GPU 主要應用於哪些 AI 產品？]`）。每個問題長度請保持精準有力（建議 15~25 字元），系統會自動將其轉化為 Telegram 互動按鈕供用戶一鍵追問。\n';
