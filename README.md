...
### 🔮 術數占卜與 AI 命理套件 (2026-08-26)

**整合奇門、梅花、塔羅、八字、風水與月老姻緣！**

串接 `https://qi.david888.com/api/`，提供專業排盤與大語言模型 AI 智能解讀。

#### 功能特點

- ✅ **塔羅占卜 (`/tarot`)** - 78 張牌、6 種牌陣（單張、時間之流、鑽石、月相、馬蹄鐵、塞爾特十字），結合 AI 心理指引。
- ✅ **生辰八字2 (`/bazi`)** - 四柱排盤、十神、日主五行、五行分佈統計與 AI 大師深度解盤。
- ✅ **八宅風水 (`/fengshui`)** - 八宅宅卦分析、本命卦、九運與流年飛星吉凶評估與佈局建議。
- ✅ **月老姻緣 (`/yinyuan`)** - 月老靈籤抽籤、生肖合婚契合度測算、個人桃花運與有利方位指引。
- ✅ **雙軌制智能引導** - 支援單行指令快速輸入（具備正則智能容錯）以及 LLM 自然語言對話多輪追問引導。

#### 指令列表與使用範例

| 指令 | 說明 | 範例 |
|------|------|------|
| `/tarot` | 塔羅抽牌與 AI 解讀 | `/tarot 我最近的事業發展如何？`<br>`/tarot diamond 創業評估` |
| `/bazi` | 生辰八字排盤與解盤 | `/bazi 1995-08-18 男 請問近幾年事業與財運`<br>`/bazi 1992-03-05 14:30 女 感情姻緣發展` |
| `/fengshui` | 八宅風水與流年飛星 | `/fengshui 坐北朝南 書房財位與文昌位如何佈置？`<br>`/fengshui 朝東南 客廳如何增旺財運？` |
| `/yinyuan` | 月老姻緣與感情測算 | `/yinyuan 求問今年感情與正緣指引`<br>`/yinyuan 1995 1998 我們合適嗎？`<br>`/yinyuan 1996 桃花運與有利方位` |
| `/qi` | 奇門遁甲問事 | `/qi 今天下午談合約順利嗎？` |
| `/mei` | 梅花易數占卜 | `/mei 這次面試能錄取嗎？` |
