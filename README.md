...
│
├── dist/                         # 📦 打包輸出 (部署用)
│   └── telegram.work.js         # 打包後的單一檔案
│
├── telegram.work.js              # 📄 原始單檔版本 (保留參考)
├── package.json                  # 📋 NPM 設定
...
打包完成後，將 `dist/telegram.work.js` 上傳到 Cloudflare Workers。
...
**Telegram 模組 (20%)**
- `src/telegram/telegram.js` - 訊息發送、Webhook 綁定
...
- `src/telegram/commands.js` - 指令處理器
- `src/features/weather.js` - 天氣查詢
- `src/features/stock.js` - 股票查詢
- `src/features/dictionary.js` - 字典功能
- `src/features/divination.js` - 占卜功能
- `src/features/law.js` - 法律問答
- `src/features/network.js` - IP/DNS 查詢
- `src/agent/azure.js` - Azure OpenAI
- `src/agent/workersai.js` - Cloudflare Workers AI
- `src/agent/mistralai.js` - Mistral AI
- `src/agent/cohere.js` - Cohere
- `src/agent/anthropic.js` - Anthropic Claude
- `src/agent/llm.js` - LLM 通用邏輯
