...
### 💡 AI 即時智慧續問與延伸追問按鈕 (Stateless AI Follow-up Suggestions) (2026-09-01)

**支援 LLM 在日常對話或專業分析完畢後，即時根據上下文自動生成 2~4 個具深度、延伸性的自然語言專屬續問按鈕！**

使用者無需手動輸入，直接點擊下方按鈕即可一鍵追問，無縫延續多輪對話上下文！

```text
┌──────────────────────────────────────────────────┐
│ 📱 Apple 近期 3nm A 系列晶片在高階 iPhone 出貨... │
├──────────────────────────────────────────────────┤
│ 🤖 NVIDIA 3nm GPU 主要應用於哪些 AI 加速器？       │
├──────────────────────────────────────────────────┤
│ 📈 若台積電 3nm 良率提升 5%，對毛利率影響？       │
├──────────────────────────────────────────────────┤
│ 🌍 在地緣政治風險下，台積電如何調整海外產能布局？ │
└──────────────────────────────────────────────────┘
```

#### 功能特點

- 🛡️ **突破 Telegram 64-Byte Callback 限制（Stateless 零 KV 架構）**：
  - 傳統伺服器快取方案（如 MD5 快取或 KV 存儲）在 Serverless 環境下容易冷啟動失效或迅速耗盡 Cloudflare KV 免費寫入配額。
  - 本專案採用 **Telegram 原生 Webhook Payload 無狀態路由**：`callback_data` 僅帶短索引（如 `ask:0`），點擊時直接自 Telegram 原始訊息之 `reply_markup` 陣列還原完整長問句，**0 次 KV 寫入，完全不佔用任何 Cloudflare KV 額度**！
- ✂️ **超長問題自動截斷保護 (`...`)**：
  - 若 LLM 生成的續問題目過長（超過 32 字元），系統自動在按鈕視覺標籤末尾加上 `...` 截斷，確保在手機版 Telegram 畫面排版工整美觀。
- 🔄 **全新訊息獨立延續對話**：
  - 點擊續問按鈕時，Bot 會發送一則全新回覆（保留歷史對話脈絡與舊訊息按鈕），並在解答完畢後繼續生成下一階段的深度追問。
- 🎯 **跨領域智慧生成**：
  - 涵蓋股市分析、時事查證、占卜算命、法律問答、程式開發與日常聊天等所有主題。
...
- 🔮 **命理占卜與決策** (`/tarot`, `/bazi`, `/fengshui`, `/yinyuan`, `/qi`, `/mei`, `/oracle`, `/boa`, `/bo`) — 塔羅、八字排盤、八宅飛星風水、月老合婚、奇門遁甲、梅花易數、解答之書。
- ⚖️ **台灣法律問答** (`/law`) — 司法法規與最高法院實務判決要旨檢索。
- 🛠️ **網路工具與字典** (`/ip`, `/dns`, `/dns2`, `/dictcn`, `/dicten`, `/password`) — IP 地理定位、DoH DNS 解析、中英辭典、隨機密碼生成。
- 👥 **代理人跨域協同** (`/delegate`) — Agent-to-Agent 跨代理人任務調度。
...
- ✅ **零幻覺自主查證** - 遇未知即時事實、公司上市狀態、時事新聞，LLM 自動發起背景查證。
...
串接 `http://dns.glsoft.ai:6000/api/analysis`，提供完整的質化與量化多空評級，並模擬投資委員會多輪辯論達成共識與倉位決策。

#### 功能特點

- ✅ **14 位傳奇投資大師評級** - 價值派（巴菲特、蒙格、葛拉漢、艾克曼）、成長派（木頭姐、費雪、林區）、反向派（麥可·貝瑞、裴洛西、WSB）、量化技術與基本面。
- ✅ **圓桌會議辯論機制** - 模擬各派大師互相交鋒、提煉會議共識觀點與多空分歧重點。
- ✅ **最終委員會多空決策** - 提供買入 (BUY)、做空 (SHORT) 或觀望 (HOLD) 建議、信心度評分與建議股數。
- ✅ **全球多市場支援** - 支援美股代號（`NVDA`, `TSLA`）、台股（`2330.TW` 或輸入 `2330` 自動識別）、港股與多標的組合分析（`AAPL,MSFT`）。

#### 指令範例

| 指令 | 說明 | 範例 |
|------|------|------|
| `/fund` | AI 對沖基金投資決策與圓桌辯論 | `/fund NVDA` （分析輝達）<br>`/fund TSLA` （分析特斯拉）<br>`/fund 2330.TW` （分析台積電）<br>`/fund AAPL,MSFT` （多標的組合分析）<br>`/fund NVDA all` （召集全部 14 位投資大師） |

---

### 🔮 7 大正統術數與 AI 命理全息套件 (2026-08-27)

**深度對齊 `tbdavid2019/qimen` 權威規範，整合奇門遁甲、梅花易數、韋特塔羅、生辰八字、易經風水、月老姻緣與解答之書！**

串接 `https://qi.david888.com/api/`，全量實作正統算法與全息數據庫：

#### 功能特點

- ✅ **奇門遁甲 (`/qi`)** - 十天干克應格局（青龍返首、飛鳥跌穴等）、三遁九遁吉格、門迫宮迫、8 大專題用神（求財/工作/感情/考試/健康/出行/官司）與主客動靜攻守策略。
- ✅ **梅花易數 (`/mei`)** - 支援時間起卦、數字起卦（3個數字）、漢字報字起卦（`字:平安`），五卦全息（本卦、互卦、變卦、錯卦、綜卦）與 384 爻動爻爻辭。
- ✅ **韋特塔羅 (`/tarot`)** - 78 張牌（大阿卡納 + 小阿卡納）、6 種牌陣（單張、時間之流、鑽石、月相、馬蹄鐵、塞爾特十字）與三大解讀變體（時間之流、現狀阻力、關係抉擇）。
- ✅ **生辰八字2 (`/bazi`)** - 四柱排盤、十神藏干、神煞體系、日主旺衰格局（正官/七殺/財/印/食傷/建祿/陽刃等）、五行分佈、姓名、曾用名、出生地與公曆/農曆雙曆法。
- ✅ **易經風水 (`/fengshui`)** - 三元玄空 24 山飛星、八宅明鏡四吉四凶、8 大內外形煞化解（路沖/天斬/壁刀/反弓/穿堂/橫梁/鏡對床）與協紀辨方擇日。
- ✅ **月老姻緣 (`/yinyuan`)** - 100 支月老靈籤（支援自選籤號 1-100 或搖籤）、生肖合婚契合度、紫微夫妻宮、個人桃花運勢時效範圍、八字合婚與紅線正緣畫像測算。
- ✅ **解答之書 (`/boa` / `/bo`)** - 支援心靈默念翻頁或具體問題 AI 深度洞察解讀雙模式。

#### 指令列表與使用範例

| 指令 | 說明 | 範例 |
|------|------|------|
| `/qi` | 奇門遁甲問事與專題用神 | `/qi 今天下午商務談判運勢如何？`<br>`/qi 求財 這筆投資項目是否可行？` |
| `/mei` | 梅花易數（時間/數字/漢字起卦） | `/mei 這次面試能錄取嗎？`<br>`/mei 12 34 56 是否適合換工作？`<br>`/mei 字:平安 是否能順利過關？` |
| `/tarot` | 塔羅抽牌與 AI 深度解讀 | `/tarot 我最近的事業發展如何？`<br>`/tarot diamond 創業評估與阻力`<br>`/tarot celtic 感情與未來展望` |
| `/bazi` | 生辰八字排盤與大師解盤 | `/bazi 1995-08-18 男 請問近幾年事業與財運`<br>`/bazi 1992-03-05 14:30 女 姓名:林小美 感情姻緣`<br>`/bazi 1988-11-20 男 農曆 創業時機評估` |
| `/fengshui` | 八宅玄空飛星、形煞與擇日 | `/fengshui 坐北朝南 書房財位與文昌位如何佈置？`<br>`/fengshui 客廳大門正對陽台穿堂煞如何化解？`<br>`/fengshui 2026年10月 入宅搬家吉日良辰` |
| `/yinyuan` | 月老姻緣・六大感情測算 | `/yinyuan 求問今年感情與正緣指引`<br>`/yinyuan 第58籤 感情復合指引`<br>`/yinyuan 1995 1998 我們合適嗎？`<br>`/yinyuan 1996 桃花運與有利方位`<br>`/yinyuan 紫微夫妻宮 1994-06-12 男 配偶特質` |
| `/boa` | 解答之書（翻閱 / 問題解讀） | `/boa` （默念翻頁）<br>`/boa 我該接受這個工作 offer 嗎？` （問題洞察解讀） |
